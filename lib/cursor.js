  ...
  if(self.cursorState.cursorId == null) {
    ...
    return setCursorDeadAndNotified(self, callback);
      ...
      self._getmore(function(err, doc) {
        ...
        // AR: sometimes raw mode loses the currentState.cursorId causing the second and subsequent batches
        // to never be fetched.  This happens only on nextBatch, never on the firstBatch returned.
        // This kludgy patch fixes up the cursorId so the entire dataset is returned.
        // AR: sometimes raw mode loses the currentState.cursorId causing the second and subsequent batches
        // to never be fetched.  This kludgy patch fixes up the cursorId so the entire dataset is returned.
        if (self.cursorState.cmd.raw && doc.length == 1 && doc[0] && doc[0].ok === 1 && doc[0].cursor &&
            Object.keys(doc[0]).length == 2 && doc[0].cursor.nextBatch && typeof doc[0].cursor.id === 'number')
        {
          self.cursorState.cursorId = Long.fromNumber(doc[0].cursor.id);
          self.cursorState.documents = doc[0].cursor.nextBatch;
          self.cursorState.cursorIndex = 0;
        }
